...
function resolveImageUrl(apiBase: string | undefined, value?: string) {
  ...
  // Keep local Next.js public assets intact (used in demo mode, too).
...
export default function AgriFoodInsightsSection() {
  const apiBase = process.env.NEXT_PUBLIC_API_URL;
  const [items, setItems] = useState<InsightItem[]>([]);
  const [agriCategorySlug, setAgriCategorySlug] = useState<string | null>(null);
  ...
  const demoInsights: InsightItem[] = useMemo(
    () => [
      {
        _id: 'demo-featured',
        slug: 'demo-agri-food-featured',
        title: 'Demo insight: Agri-food decision intelligence',
        metaDescription:
          'Sample content for layout while we wire the final Agri-food feed.',
        content:
          '<p>Demo insight content about evidence-led agriculture decision intelligence.</p>',
        featuredImage: '/images/agri-why-client.jpg',
        insightCategorySlug: 'agri-food',
        createdAt: new Date().toISOString(),
      },
      {
        _id: 'demo-2',
        slug: 'demo-agri-food-2',
        title: 'Demo insight: Market & opportunity research for AgTech',
        metaDescription: 'Sample insight card displayed until real CMS content is connected.',
        content: '<p>Demo insight content for market and opportunity research.</p>',
        featuredImage: '/images/agri-why-client.jpg',
        insightCategorySlug: 'agri-food',
      ...
      {
        _id: 'demo-3',
        slug: 'demo-agri-food-3',
        title: 'Demo insight: Monitoring, evaluation and learning systems',
        metaDescription: 'Sample insight card displayed until real CMS content is connected.',
        content: '<p>Demo insight content for M&E and learning systems.</p>',
        featuredImage: '/images/agri-why-client.jpg',
        insightCategorySlug: 'agri-food',
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const findAgriCategory =
          categoriesList.find(
            (c) =>
              normalized(c.name).includes('agri') &&
              (normalized(c.name).includes('food') || normalized(c.slug).includes('food')),
          ) ||
          categoriesList.find(
            (c) => normalized(c.slug).includes('agri') && normalized(c.slug).includes('food'),
          ) ||
          categoriesList.find((c) => normalized(c.slug).includes('agri'));

        const slug = findAgriCategory?.slug || null;
        setAgriCategorySlug(slug);
  ...
  const categoryLabel = useMemo(() => {
    if (!agriCategorySlug) return 'Agri-food';
    return agriCategorySlug.replace(/-/g, ' ');
  }, [agriCategorySlug]);
    ...
    <section className="py-16 md:py-20 bg-white">
      <div className="container-custom">
        <div className="space-y-3 mb-10">
          ...
          <h2 className="text-4xl font-bold text-gray-900">Agri-food research & analysis</h2>
        ...
        {/* Featured card */}
        <div className="relative rounded-3xl overflow-hidden" style={{ boxShadow: '0px 25px 50px rgba(0, 0, 0, 0.25)' }}>
